...
const MyEvents = () => {
    ...
    const fetchMyEvents = async () => {
        try {
            setLoading(true);
            const response = await fetch(`http://localhost:5000/api/v1/events/organizer/${user._id}`, {
                method: 'GET',
                headers: {
                ...
                {events.map(event => (
                    <div className="col" key={event._id}>
                        <div className="card h-100 shadow-sm">
                            <div className={`card-status-badge badge bg-${statusColors[event.status]}`}>
                                {event.status}
                            </div>
                            ...
                            <div className="card-footer bg-white border-top-0">
                                ...
                                {event.status === 'Declined' && (
                                    <div className="mt-2 small text-danger">
                                        ...
                                        Your event was declined. Please check your email for details.
                                    </div>
                                )}
                            </div>
                        </div>
